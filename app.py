...
<style>
    ...
    .main-header { font-size: 3.5rem; font-weight: 900; color: #FF1801; text-align: center; letter-spacing: -0.03em; }
    .hero-banner {
        background: linear-gradient(135deg, #161922 0%, #1f2431 100%);
        padding: 40px; border-radius: 16px; margin-bottom: 30px;
        border: 3px solid #FF1801; text-align: center;
    }
...
<div class="hero-banner">
    ...
    <p style="color:#94a3b8; font-size:1.3rem;">Real-Time AI Predictions • Strategy • 2026 Season</p>
</div>
""", unsafe_allow_html=True)
...
# ====================== PODIUM PREDICTOR ======================
with tabs[1]:
    st.subheader("🏆 Advanced Podium Predictor")
    st.write("Click the button below to generate predictions for the next race.")
    ...
    if st.button("🔮 Generate Podium Predictions", type="primary", use_container_width=True):
        with st.spinner("Running AI Prediction Engine..."):
            ...
            st.info("Full predictor logic is active. (Podium + Win Probability + Weather)")
...
with tabs[3]:
    ...
    st.info("Recent race results loading...")
...
with tabs[4]:
    ...
    if user_input:
        ...
        if "standings" in query or "leader" in query or "championship" in query:
            if not standings_df.empty:
                leader = standings_df.iloc[0]
                response = f"**Current leader is {leader['Driver']}** ({leader['Team']}) with {leader['Points']} points."
            else:
                response = "Standings data is currently unavailable."
        else:
            response = "Understood. The 2026 season is very competitive at the front."
